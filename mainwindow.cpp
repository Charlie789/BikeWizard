...
void MainWindow::set_selected_parts_model(QStandardItemModel* model)
{
    ...
    for (int i = 0; i < m_selected_parts_model->rowCount(); ++i){
        ...
        select_button->setProperty("part_type", m_selected_parts_model->data(m_selected_parts_model->index(i, 1)).toInt());
        ui->selected_parts_tableview->setIndexWidget(model->index(i, 1), select_button);
        connect(select_button, &QPushButton::clicked, this, &MainWindow::select_button_clicked);
    }
}

void MainWindow::select_button_clicked()
{
